...
export async function duyetBaoNghi(donId, data, ghiChuTK = "") {
  try {
    ...
    // Cập nhật daDay = 0 cho tuần báo nghỉ trong môn học
    const { data: monHoc } = await api.get(`/monHoc/${donBaoNghi.idMonHoc}`);
    const cacTuanUpdated = monHoc.cacTuan.map((tuan) => {
      if (tuan.tuan === donBaoNghi.tuan) {
        return { ...tuan, daDay: 0, trangThai: "da_duyet" };
      }
      return tuan;
    });
    await api.put(`/monHoc/${donBaoNghi.idMonHoc}`, {
      ...data,
      cacTuan: cacTuanUpdated,
    });
